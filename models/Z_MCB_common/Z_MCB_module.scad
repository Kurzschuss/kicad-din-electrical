...
mcb_front_step_height = 18.0;

mcb_toggle_width = 10.0;
mcb_toggle_length = 18.0;
mcb_toggle_height = 8.0;
mcb_toggle_angle = -12;
...
module mcb_housing() {
    difference() {
        mcb_rounded_box_xy([mcb_module_width, mcb_module_length, mcb_body_depth], 1.0);
        for (sy = [-1, 1]) {
            translate([0, sy*(mcb_module_length/2-mcb_terminal_length/2), mcb_body_depth-mcb_terminal_recess_depth/2])
...
module mcb_terminal_collar(y0) {
    translate([0, y0, mcb_body_depth-2.0])
        difference() {
...
module mcb_poles(pole_count=1) {
    ...
    if (pole_count > 1) {
        translate([0, 0, mcb_body_depth + mcb_front_step_height + 4.0])
