"""Separates Audit abgelehnter Bearbeitungsversuche fuer Alarme aus AP-0105."""
...
class GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryActionAttemptRecord:
...
class SQLiteGlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryActionAttemptRepository:
    ...
    def append(self,record):
        if record.attempted_at.tzinfo is None or record.attempted_at.utcoffset() is None: raise ValueError("ERR-KICAD-0294: Der Versuchszeitpunkt benoetigt eine Zeitzone.")
        code=record.denial_code.strip(); reason=record.denial_reason.strip()
        if not code: raise ValueError("ERR-KICAD-0295: Der Ablehnungscode fehlt.")
        if not reason: raise ValueError("ERR-KICAD-0296: Die Ablehnungsbegruendung fehlt.")
        try:
        ...
        except sqlite3.IntegrityError as exc: raise ValueError("ERR-KICAD-0297: Die Versuchskennung ist bereits vorhanden.") from exc
        return record
    def list_for_alert(self,alert_id):
        ...
        return tuple(GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryActionAttemptRecord(BusinessId(r[0]),BusinessId(r[1]),Action(r[2]),datetime.fromisoformat(r[3]),BusinessId(r[4]) if r[4] else None,BusinessId(r[5]),BusinessId(r[6]),r[7],r[8],CorrelationId(r[9])) for r in rows)
