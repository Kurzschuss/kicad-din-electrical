"""Persistente Alarmereignisse fuer die Alarmbewertung aus AP-0104."""
...
from .kicad_global_security_release_alert_attempt_action_attempt_action_attempt_alert import GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptAlertLevel as Level, GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptAlertResult as Result

class GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryStatus(StrEnum):
    OPEN="OPEN"; ACKNOWLEDGED="ACKNOWLEDGED"; RESOLVED="RESOLVED"

@dataclass(frozen=True,slots=True)
class GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryRecord:
    alert_id:BusinessId; level:Level; status:GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryStatus
...
class SQLiteGlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryRepository:
    TABLE="projectos_global_security_staffing_release_alert_attempt_action_attempt_alert_action_attempt_alerts"
    def __init__(self,connection:sqlite3.Connection):
        ...
        connection.execute(f"CREATE INDEX IF NOT EXISTS idx_ap0105_status ON {self.TABLE}(status,created_at DESC,alert_id DESC)"); connection.commit()
    def create(self,*,alert_id:BusinessId,result:Result,correlation_id:CorrelationId):
        if not result.alert: raise ValueError("ERR-KICAD-0279: Ein CLEAR-Ergebnis darf nicht gespeichert werden.")
        for value in (result.evaluated_at,result.window_start):
            if value.tzinfo is None or value.utcoffset() is None: raise ValueError("ERR-KICAD-0280: Alarmzeitpunkte benoetigen eine Zeitzone.")
        try:
        ...
        except sqlite3.IntegrityError as exc: raise ValueError("ERR-KICAD-0281: Die Alarmkennung ist bereits vorhanden.") from exc
    ...
    def acknowledge(self,alert_id:BusinessId,*,acknowledged_at:datetime,acknowledged_by:BusinessId,reason:str):
        ...
        if record.status is not GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryStatus.OPEN: raise ValueError("ERR-KICAD-0282: Nur ein offener Alarm kann bestaetigt werden.")
        instant,normalized=_validate_action(acknowledged_at,reason)
        if instant<record.created_at: raise ValueError("ERR-KICAD-0283: Die Bestaetigung liegt vor der Alarmerzeugung.")
    ...
    def resolve(self,alert_id:BusinessId,*,resolved_at:datetime,resolved_by:BusinessId,reason:str):
        record=self.get(alert_id)
        if record.status is not GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryStatus.ACKNOWLEDGED: raise ValueError("ERR-KICAD-0284: Nur ein bestaetigter Alarm kann abgeschlossen werden.")
        instant,normalized=_validate_action(resolved_at,reason)
        if record.acknowledged_at is None or instant<record.acknowledged_at: raise ValueError("ERR-KICAD-0285: Der Abschluss liegt vor der Bestaetigung.")
    ...
    def get(self,alert_id:BusinessId):
        ...
        if row is None: raise ValueError("ERR-KICAD-0286: Sicherheitsalarm wurde nicht gefunden.")
        return _decode(row)
    def list_for_status(self,status:GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryStatus):
...
def _validate_action(at:datetime,reason:str):
    ...
    if not normalized: raise ValueError("ERR-KICAD-0288: Die Alarmbearbeitung benoetigt eine Begruendung.")
    return at,normalized

def _decode(row):
    S=GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryStatus
    return GlobalSecurityStaffingReleaseAlertAttemptHistoryActionAttemptHistoryActionAttemptHistoryRecord(BusinessId(str(row[0])),Level(str(row[1])),S(str(row[2])),datetime.fromisoformat(str(row[3])),datetime.fromisoformat(str(row[4])),int(row[5]),int(row[6]),int(row[7]),int(row[8]),tuple(json.loads(str(row[9]))),CorrelationId(str(row[10])),datetime.fromisoformat(str(row[11])) if row[11] else None,BusinessId(str(row[12])) if row[12] else None,str(row[13]) if row[13] else None,datetime.fromisoformat(str(row[14])) if row[14] else None,BusinessId(str(row[15])) if row[15] else None,str(row[16]) if row[16] else None)
