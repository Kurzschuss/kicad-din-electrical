#!/usr/bin/env python3
"""Exportiert und prüft das ProjectOS-eigene MCB-4P-3D-Modell."""
...
MODEL_DIR = REPO_ROOT / "models" / "Z_MCB_4P"
SOURCE = MODEL_DIR / "Z_MCB_4P.scad"
OUTPUT_DIR = MODEL_DIR / "generated"
STEP_OUTPUT = OUTPUT_DIR / "Z_MCB_4P.step"
WRL_OUTPUT = OUTPUT_DIR / "Z_MCB_4P.wrl"
EXPECTED_WIDTH_MM = 72.0
...
def export_model(toolchain: Toolchain, *, output_dir: Path = OUTPUT_DIR) -> tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="projectos-z-mcb-4p-") as temp_dir:
        temp = Path(temp_dir)
        stl = temp / "Z_MCB_4P.stl"
...
def validate_bounds(label: str, x: float, y: float) -> None:
    if abs(x - EXPECTED_WIDTH_MM) > GEOMETRY_TOLERANCE_MM:
        raise RuntimeError(f"{label}-Breite {x:.4f} mm statt {EXPECTED_WIDTH_MM:.4f} mm")
    if abs(y - EXPECTED_LENGTH_MM) > GEOMETRY_TOLERANCE_MM:
        raise RuntimeError(f"{label}-Länge {y:.4f} mm statt {EXPECTED_LENGTH_MM:.4f} mm")


def check_geometry(toolchain: Toolchain) -> None:
    with tempfile.TemporaryDirectory(prefix="projectos-z-mcb-4p-geometry-") as temp_dir:
    ...
    print("MCB-4P-3D-Maßhaltigkeitsprüfung erfolgreich.")
...
def main() -> int:
    ...
    try:
        ...
        print(f"MCB-4P-3D-Export fehlgeschlagen: {exc}", file=sys.stderr)
        return 1
