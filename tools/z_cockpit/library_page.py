...
def library_page_html(libraries: tuple[SymbolLibrary, ...] | None = None) -> str:
    ...
    return (
        '<section class="page" id="page-bibliotheken"><h2>Bibliotheken</h2>'
        ...
        f'<div class="card">Vorschaupaare<strong>{total_complete}</strong></div></div>'
        f'<div class="library-list">{"".join(cards)}</div></section>'
    )
