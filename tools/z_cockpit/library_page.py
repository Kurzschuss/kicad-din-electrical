...
def _symbol_table(library: SymbolLibrary) -> str:
    ...
    return (
        '<div class="library-symbol-table-wrap"><table class="library-symbol-table">'
...
def library_page_html(libraries: tuple[SymbolLibrary, ...] | None = None) -> str:
    ...
    return (
        ...
        '.library-main,.library-details{min-width:0;min-height:0;padding:1rem;overflow:auto}'
        '.library-main{display:flex;flex-direction:column}'
        '.library-details{border-left:1px solid #8886}'
        ...
        '.library-result-count{margin:.65rem 0 0;font-size:.9rem;opacity:.8}'
        '.library-properties{margin-top:0}'
        '.library-symbol-table-wrap{overflow:auto;border:1px solid #8886;border-radius:.35rem}'
        '.library-symbol-table{border-collapse:collapse;width:100%;min-width:780px}'
        '.library-symbol-table th,.library-symbol-table td{padding:.5rem .6rem;'
        'border-bottom:1px solid #8884;text-align:left;white-space:nowrap}'
        '.library-symbol-table thead th{position:sticky;top:0;background:Canvas}'
        '.library-symbol-table th[scope="row"]{position:static;background:transparent}'
        ...
        '<div class="library-main"><h3>Bibliotheksliste</h3>'
        '<div class="library-filters">'
        '<label>Bibliothek<select id="library-filter-name"><option value="">Alle</option>'
        f'{_select_options(library_names)}</select></label>'
        '<label>Symbole vorhanden<select id="library-filter-symbols"><option value="">Alle</option>'
        '<option>Ja</option><option>Nein</option></select></label>'
        '<label>Gerätezuordnung<select id="library-filter-devices"><option value="">Alle</option>'
        '<option>Ja</option><option>Nein</option></select></label>'
        '<label>Footprints<select id="library-filter-footprints"><option value="">Alle</option>'
        '<option>Ja</option><option>Nein</option></select></label>'
        '<label>Vorschauen<select id="library-filter-preview"><option value="">Alle</option>'
        ...
        '<div id="library-detail-content"><p>Bitte eine Bibliothek auswählen.</p></div>'
        f'{"".join(detail_templates)}</section></div>'
        '</section>'
        '<script>'
        '(()=>{'
