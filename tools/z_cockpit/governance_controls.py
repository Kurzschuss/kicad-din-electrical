...
function buildUserAdmin(){{
 ...
 function renderMatrix(u){{const box=document.getElementById('gov-user-matrix');if(!u){{box.innerHTML='<p style="padding:.6rem">Benutzer auswählen, um Rechte und Zugriffsbereiche zu sehen.</p>';return}}let html='<table><thead><tr><th>Projekt-Recht</th><th>Status</th></tr></thead><tbody>';u.project_rights.forEach(r=>{{html+=`<tr><td><code>${{h(r.permission)}}</code><br><small>${{h(r.label)}}</small></td><td>${{pill(r.decision)}}</td></tr>`}});html+='</tbody></table><table><thead><tr><th>Bereich</th><th>Sehen</th><th>Bearbeiten</th></tr></thead><tbody>';u.page_rights.forEach(r=>{{html+=`<tr><td>${{h(r.label)}}</td><td>${{pill(r.view)}}</td><td>${{pill(r.edit)}}</td></tr>`}});html+='</tbody></table>';box.innerHTML=html}}
...
function buildAutoReporting(){{
 ...
 const reasons=gate.allowed?'Alle Voraussetzungen erfüllt.':gate.reasons.map(x=>`• ${{h(x)}}`).join('<br>');
 let last='';if(M.last_report&&M.last_report.status){{const reporters=(M.last_report.reporters||[]).map(h).join(', ')||'–';last=`<div class="gov-report-last"><strong>Letzte Automatikprüfung:</strong> ${{h(M.last_report.message||M.last_report.status)}}<br>Issue: ${{M.last_report.issue_number?'#'+Number(M.last_report.issue_number):'–'}} · Meldungen: ${{Number(M.last_report.report_count||0)}} · Reporter: ${{reporters}}</div>`}}
 box.innerHTML=`<strong>Automatische GitHub-Meldung: ${{gate.allowed?'freigegeben':'gesperrt'}}</strong><br>GitHub/ProjectOS: ${{h(user)}}<br>Repository: ${{h(gate.repository_status)}}<br><span>${{reasons}}</span>${{last}}<div class="gov-actions" style="margin-top:.6rem"><button type="button" id="gov-auto-send" ${{gate.allowed?'':'disabled'}}>Nach Dublettenprüfung automatisch senden</button></div><p class="gov-note">Vor jedem Schreibzugriff werden Version, offizielles Repository/Fork, GitHub-Authentifizierung, ProjectOS-Zuordnung, Benutzerstatus und <code>github.issue.auto_submit</code> lokal vertrauenswürdig erneut geprüft. Bereits bekannte Fehler erzeugen kein zweites Issue; ProjectOS ergänzt eine Wiederholungsmeldung und zählt Reporter/Meldungen.</p>`;
