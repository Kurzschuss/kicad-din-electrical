...
def settings_page_html(snapshot: CockpitSettingsSnapshot | None = None) -> str:
    ...
    return (
        ...
        '.cockpit-page-title{margin:0 0 .85rem;flex:0 0 auto}'
        '.cockpit-page-title small{font-size:.62em;font-weight:400;opacity:.75}'
        '#page-sicherheit .security-table-wrap{margin-top:0}'
        ...
        '<h2 class="cockpit-page-title">Einstellungen '
        '<small class="cockpit-page-description">(Projektwerte aus Repository-Quellen; lokale Oberflächenoptionen nur in diesem Browser.)</small></h2>'
        ...
        'function compactPageHeadings(){document.querySelectorAll(".page").forEach(page=>{const heading=page.querySelector("h2");if(!heading||heading.classList.contains("cockpit-page-title"))return;const description=heading.nextElementSibling;if(!description||description.tagName!=="P")return;const text=description.textContent.trim();if(!text)return;const small=document.createElement("small");small.className="cockpit-page-description";small.textContent=`(${text})`;heading.append(" ",small);heading.classList.add("cockpit-page-title");description.remove();});}'
        'let state=read();'
        ...
        'if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",compactPageHeadings,{once:true})}else{compactPageHeadings()}'
        'apply();'
        '})();</script>'
    )
