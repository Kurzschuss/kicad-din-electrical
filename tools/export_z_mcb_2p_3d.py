#!/usr/bin/env python3
"""Exportiert und prüft das ProjectOS-eigene MCB-2P-3D-Modell."""
...
MODEL_DIR = REPO_ROOT / "models" / "Z_MCB_2P"
SOURCE = MODEL_DIR / "Z_MCB_2P.scad"
OUTPUT_DIR = MODEL_DIR / "generated"
STEP_OUTPUT = OUTPUT_DIR / "Z_MCB_2P.step"
WRL_OUTPUT = OUTPUT_DIR / "Z_MCB_2P.wrl"
EXPECTED_WIDTH_MM = 36.0
...
def export_model(toolchain: Toolchain, *, output_dir: Path = OUTPUT_DIR) -> tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="projectos-z-mcb-2p-") as temp_dir:
        temp = Path(temp_dir)
        stl = temp / "Z_MCB_2P.stl"
...
def check_geometry(toolchain: Toolchain) -> None:
    with tempfile.TemporaryDirectory(prefix="projectos-z-mcb-2p-geometry-") as temp_dir:
    ...
    print("MCB-2P-3D-Maßhaltigkeitsprüfung erfolgreich.")
...
def main() -> int:
    ...
    try:
        ...
        print(f"MCB-2P-3D-Export fehlgeschlagen: {exc}", file=sys.stderr)
        return 1
