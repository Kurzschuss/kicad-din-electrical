...
def cockpit_devices() -> list[dict[str, object]]:
    ...
    for item in collect_devices()["devices"]:
        ...
        three_d = three_d_preview_assignment(symbol)
        result.append({
            ...
            "model": three_d.model_available,
            "model_reference": three_d.model_reference or "",
            "three_d_preview_url": three_d.preview_relative_url,
            "three_d_preview_available": three_d.preview_available,
            "three_d_preview_status": three_d.preview_status,
...
def cockpit_summary(devices: list[dict[str, object]]) -> dict[str, int]:
    return {
        ...
        "models": sum(bool(item["model"]) for item in devices),
        "three_d_previews": sum(bool(item["three_d_preview_available"]) for item in devices),
    }
...
<section class="page active" id="page-start"><h2>Projektstatus</h2><p>Zentrale Übersicht aus Gerätekatalog und Projektmodell.</p><div class="cards"><div class="card">Geräte<strong>{summary['devices']}</strong></div><div class="card">Gerätefamilien<strong>{summary['families']}</strong></div><div class="card">Hersteller<strong>{summary['manufacturers']}</strong></div><div class="card">Geprüfte Geräte<strong>{summary['checked']}</strong></div><div class="card">3D-Modelle<strong>{summary['models']}</strong></div><div class="card">3D-Vorschauen<strong>{summary['three_d_previews']}</strong></div></div><div class="dashboard-grid"><section class="dashboard-panel"><h3>Fortschritt bis Version {project.target_release}</h3>{project_progress_html(project)}</section><section class="dashboard-panel"><h3>Nächste Aufgaben</h3>{next_tasks_html(project, limit=5)}</section></div>{development_navigator_html(project)}<section class="status-section"><h3>Projektbestandteile</h3><div class="status-grid">{project_status_html()}</div></section></section>
<section class="page" id="page-geraete"><div class="device-layout"><div class="device-main"><h2>Geräte</h2><div class="filters"><label>Gerätefamilie<select id="family"><option value="">Alle</option></select></label><label>Hersteller<select id="manufacturer"><option value="">Alle</option></select></label><label>Polzahl<select id="poles"><option value="">Alle</option></select></label><label>Charakteristik<select id="curve"><option value="">Alle</option></select></label><label>Nennstrom<select id="current"><option value="">Alle</option></select></label><label>RCD-Typ<select id="rcd_type"><option value="">Alle</option></select></label><label>IΔn<select id="residual_current"><option value="">Alle</option></select></label><label>Status<select id="status"><option value="">Alle</option></select></label></div><div class="table-wrap"><table id="devices"><thead><tr><th>Name</th><th>Technische ID</th><th>Familie</th><th>Hersteller</th><th>Polzahl</th><th>Charakteristik</th><th>Nennstrom</th><th>RCD-Typ</th><th>IΔn</th><th>Symbol</th><th>Footprint</th><th>3D</th><th>Status</th></tr></thead><tbody></tbody></table></div></div><section class="details"><h2>Eigenschaften</h2><dl id="properties"><dt>Auswahl</dt><dd>Bitte ein Gerät auswählen.</dd></dl><div class="preview-grid"><article class="preview-card"><h3>Symbol</h3><div class="preview" id="symbol-preview">Vorschau wird nach Auswahl angezeigt.</div></article><article class="preview-card"><h3>Footprint</h3><div class="preview" id="footprint-preview">Vorschau wird nach Auswahl angezeigt.</div></article><article class="preview-card"><h3>3D</h3><div class="preview" id="three-d-preview">Vorschau wird nach Auswahl angezeigt.</div></article></div></section></div></section>{library_page_html()}{manufacturer_page_html()}{library_health_page_html()}{diagnostics_page_html()}{user_management_page_html(users)}{security_page_html()}{documentation_page_html()}{settings_page_html()}{placeholder_pages_html()}</div></main>
<footer><span id="count">{summary['devices']} Gerät(e)</span><span>Datenquellen: Gerätekatalog, KiCad-Bibliotheken, 3D-Modellreferenzen, project_state.yaml und ProjectOS</span><span>Z_Cockpit 1.2</span></footer>
<script>const data={payload};const fields={{family:'family',manufacturer:'manufacturer',poles:'poles',curve:'curve',current:'current',rcd_type:'rcd_type',residual_current:'residual_current',status:'status'}};function showPage(id){{document.querySelectorAll('.page').forEach(x=>x.classList.toggle('active',x.id===`page-${{id}}`));document.querySelectorAll('.page-link').forEach(x=>x.classList.toggle('active',x.dataset.page===id));}}document.querySelectorAll('.page-link').forEach(button=>button.addEventListener('click',()=>showPage(button.dataset.page)));document.querySelector('[data-page="start"]').classList.add('active');function values(key){{return[...new Set(data.map(x=>x[key]))].sort((a,b)=>a.localeCompare(b,'de',{{numeric:true}}))}}function option(select,value){{const o=document.createElement('option');o.value=value;o.textContent=value;select.appendChild(o)}}Object.entries(fields).forEach(([id,key])=>{{const el=document.getElementById(id);values(key).forEach(v=>option(el,v));el.addEventListener('change',render)}});function render(){{const tbody=document.querySelector('#devices tbody');tbody.innerHTML='';const rows=data.filter(item=>Object.entries(fields).every(([id,key])=>{{const v=document.getElementById(id).value;return!v||item[key]===v}}));rows.forEach(item=>{{const tr=document.createElement('tr');tr.innerHTML=`<td>${{item.name}}</td><td><code>${{item.id}}</code></td><td>${{item.family}}</td><td>${{item.manufacturer}}</td><td>${{item.poles}}</td><td>${{item.curve}}</td><td>${{item.current}}</td><td>${{item.rcd_type}}</td><td>${{item.residual_current}}</td><td><code>${{item.symbol}}</code></td><td>${{item.footprint}}</td><td>${{item.three_d_preview_status}}</td><td>${{item.status}}</td>`;tr.addEventListener('click',()=>selectRow(tr,item));tbody.appendChild(tr)}});document.getElementById('count').textContent=`${{rows.length}} Gerät(e)`}}function symbolPreviewHtml(item){{if(!item.symbol_preview_available)return `<div><strong>${{item.name}}</strong><p>Für dieses Symbol ist keine Vorschau verfügbar.</p></div>`;return `<div><img src="${{item.symbol_preview_url}}" alt="Symbolvorschau ${{item.symbol}}"><p class="preview-note">Technische SVG-Schnellansicht · ${{item.symbol}}</p></div>`}}function footprintPreviewHtml(item){{if(!item.footprint_preview_available)return `<div><strong>${{item.footprint}}</strong><p>Für diesen Footprint ist keine Vorschau verfügbar.</p><p class="preview-status">${{item.footprint_preview_status}}</p></div>`;const note=item.footprint_preview_status==='Platzhalter'?'Footprint vorhanden, aber noch ohne darstellbare Geometrie.':'Technische SVG-Schnellansicht';return `<div><img src="${{item.footprint_preview_url}}" alt="Footprintvorschau ${{item.footprint}}"><p class="preview-note">${{note}} · ${{item.footprint}}</p><p class="preview-status">${{item.footprint_preview_status}}</p></div>`}}function threeDPreviewHtml(item){{if(!item.three_d_preview_available){{let text='Kein 3D-Modell und keine technische Hüllkörper-Vorschau verfügbar.';if(item.three_d_preview_status==='Modellreferenz fehlt')text='Eine 3D-Modellreferenz ist eingetragen, die Repository-Datei fehlt oder kann nicht aufgelöst werden.';return `<div><strong>${{item.footprint}}</strong><p>${{text}}</p><p class="preview-status">${{item.three_d_preview_status}}</p></div>`}}const note=item.model?'Echtes KiCad-3D-Modell vorhanden.':'Technische Hüllkörper-Vorschau aus vorhandener F.Fab-Geometrie; kein 3D-Modell.';return `<div><img src="${{item.three_d_preview_url}}" alt="3D-Vorschau ${{item.footprint}}"><p class="preview-note">${{note}}</p><p class="preview-status">${{item.three_d_preview_status}}</p></div>`}}function selectRow(tr,item){{document.querySelectorAll('#devices tbody tr').forEach(x=>x.classList.remove('selected'));tr.classList.add('selected');const modelRef=item.model_reference?`<code>${{item.model_reference}}</code>`:'–';document.getElementById('properties').innerHTML=`<dt>Name</dt><dd>${{item.name}}</dd><dt>Technische ID</dt><dd><code>${{item.id}}</code></dd><dt>Symbol</dt><dd><code>${{item.symbol}}</code></dd><dt>Footprint</dt><dd><code>${{item.footprint}}</code></dd><dt>3D-Status</dt><dd>${{item.three_d_preview_status}}</dd><dt>3D-Modellreferenz</dt><dd>${{modelRef}}</dd><dt>Familie</dt><dd>${{item.family}}</dd><dt>Nennstrom</dt><dd>${{item.current}}</dd><dt>RCD-Typ</dt><dd>${{item.rcd_type}}</dd><dt>IΔn</dt><dd>${{item.residual_current}}</dd><dt>Status</dt><dd>${{item.status}}</dd>`;document.getElementById('symbol-preview').innerHTML=symbolPreviewHtml(item);document.getElementById('footprint-preview').innerHTML=footprintPreviewHtml(item);document.getElementById('three-d-preview').innerHTML=threeDPreviewHtml(item)}}render();</script></body></html>"""
