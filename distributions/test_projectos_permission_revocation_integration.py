...
def test_bundle_v4_roundtrip_preserves_assignment_and_revocation(tmp_path):
    ...
    assignment = ProjectOSPermissionAssignment(user_id=user.user_id, permission="project.read", source_type="direct", effect="allow")
...
def test_bundle_v4_with_user_management_v1_loads_and_explicit_save_upgrades_to_v2(tmp_path):
    ...
    manager.set_user_management(ProjectOSUserManagementState(project_id=manager.project_id, users=(user,)))
    ...
    payload["user_management"].pop("role_assignment_terminations")
    path.write_text(json.dumps(payload), encoding="utf-8")

    loaded = DinEditorProjectManager()
    loaded.load(path)
    assert loaded.user_management.permission_revocations == ()
    assert loaded.user_management.role_assignment_terminations == ()
    ...
    assert upgraded["user_management"]["role_assignment_terminations"] == []
...
def test_revoked_command_permission_blocks_later_command_without_side_effects():
    ...
    with pytest.raises(PermissionError, match="not_granted"):
        runtime.changes.change_user_weight(target.user_id, 500, command_context=_context(operator.user_id))
