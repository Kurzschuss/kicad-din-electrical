...
class DinEditorSyncActions:
    ...
    def _record(self, reference: str, source: str, value: str, action: str) -> dict:
    ...
    def keep_din(self, reference: str) -> dict:
        ...
        try:
            ...
            self._record(reference, "DIN", self._label(reference), "kept")
        except Exception:
    ...
    def use_kicad(self, reference: str) -> dict:
        ...
        try:
            ...
            self._record(reference, "KiCad", self._label(reference), "imported")
        except Exception:
            self._rollback_history(history, captured)
            raise
        return self._changed(state)
        ...
        try:
            ...
            for component in after:
                ...
                if reference in before and label != before[reference]:
                    self._record(reference, "KiCad", label, "imported")
        except Exception:
            self._rollback_history(history, captured)
            raise
        return self._changed(state)
    ...
    def resolve_all(self, choice: str) -> dict:
        ...
        try:
            ...
            for conflict in conflicts:
                ...
                self._record(reference, source, self._label(reference), action)
        except Exception:
            self._rollback_history(history, captured)
            raise
        return self._changed(state)
