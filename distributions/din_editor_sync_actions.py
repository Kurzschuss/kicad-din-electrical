...
class DinEditorSyncActions:
    ...
    def keep_din(self, reference: str, *, correlation_id: str | None = None, causation_id: str | None = None) -> dict:
        self._ensure_current()
        ...
        try:
            ...
            self._record(reference, "DIN", self._label(reference), "kept", correlation_id=correlation_id, causation_id=causation_id)
        except Exception:
            self._rollback_history(history, captured)
            raise
        return self._changed(state)

    def use_kicad(self, reference: str, *, correlation_id: str | None = None, causation_id: str | None = None) -> dict:
        self._ensure_current()
        history = self.view_model.sync_service.change_service.history
        captured = history.capture()
        try:
            state = self.view_model.choose(reference, "kicad")
            self._record(reference, "KiCad", self._label(reference), "imported", correlation_id=correlation_id, causation_id=causation_id)
        except Exception:
            self._rollback_history(history, captured)
            raise
        return self._changed(state)

    def import_manifest(self, manifest: dict, overwrite: bool = True, *, correlation_id: str | None = None, causation_id: str | None = None) -> dict:
        self._ensure_current()
        history = self.view_model.sync_service.change_service.history
        captured = history.capture()
        before = {
        ...
        try:
            ...
            for component in after:
                ...
                if reference in before and label != before[reference]:
                    self._record(reference, "KiCad", label, "imported", correlation_id=correlation_id, causation_id=causation_id)
        except Exception:
            self._rollback_history(history, captured)
            raise
        return self._changed(state)

    def resolve_all(self, choice: str, *, correlation_id: str | None = None, causation_id: str | None = None) -> dict:
        self._ensure_current()
        ...
        try:
            ...
            for conflict in conflicts:
                ...
                self._record(reference, source, self._label(reference), action, correlation_id=correlation_id, causation_id=causation_id)
        except Exception:
            self._rollback_history(history, captured)
            raise
        return self._changed(state)
