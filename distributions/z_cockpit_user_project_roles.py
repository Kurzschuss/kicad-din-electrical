...
class ZCockpitUserProjectRoleView:
    """Zeigt Funktionen, Herkunft, Beendigungen und abgeleitete Rechte read-only."""
    ...
    def __init__(
        ...
        *,
        ...
        self.user = user
        self.registry = ProjectOSUserProjectRoleRegistry(roles, role_terminations)
    ...
    def state(self, *, scope: str = "project", at: datetime | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "terminated_roles": [
                {
                    "role": self._decorate_role(item["role"], active=False),
                    "termination": dict(item["termination"]),
                }
                for item in role_state["terminated_roles"]
            ],
            "termination_count": role_state["termination_count"],
    ...
    def simulate_roles(
        ...
        *,
        ...
        baseline_terminations = tuple(self.registry._terminations)
        hypothetical = tuple(hypothetical_roles)
        baseline_derived = self.registry.permission_assignments(
            project_id=self.project_id, user=self.user, permission_map=self.permission_map, scope=scope, at=at
        )
        simulated_registry = ProjectOSUserProjectRoleRegistry(baseline_roles + hypothetical, baseline_terminations)
        ...
        return {
            ...
            "added_role_count": len(hypothetical),
            "read_only": True,
            "note": "Die Simulation ergänzt Projektfunktionen nur hypothetisch und verändert weder gespeicherte Zuordnungen noch Beendigungen.",
        }
