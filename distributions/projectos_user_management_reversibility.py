...
class ProjectOSUserManagementReversibilityRule:
    ...
    def __post_init__(self) -> None:
        operation = str(self.operation).strip(); reason = str(self.reason).strip()
        compensation = str(self.compensation).strip() if self.compensation is not None else None
        if not operation: raise ValueError("operation must not be empty")
        if not reason: raise ValueError("reversibility reason must not be empty")
        if self.reversible and not compensation: raise ValueError("reversible operation requires compensation")
        if not self.reversible and compensation is not None: raise ValueError("non-reversible operation must not define compensation")
        object.__setattr__(self, "operation", operation); object.__setattr__(self, "compensation", compensation); object.__setattr__(self, "reason", reason)
...
_DEFAULT_RULES = (
    ProjectOSUserManagementReversibilityRule("user_created", False, None, "Deaktivierung ist modelliert; für vollständiges Undo/Redo fehlt aber noch ein expliziter Reaktivierungsvertrag derselben user_id."),
    ...
    ProjectOSUserManagementReversibilityRule("user_deactivated", False, None, "Benutzer-Deaktivierung ist eine historische Lifecycle-Tatsache; Reaktivierung ist noch nicht modelliert."),
    ...
    ProjectOSUserManagementReversibilityRule("project_role_reassigned", False, None, "Eine Rollen-Neu-Zuweisung ist ein neuer Lifecycle-Vorgang mit eigener Identität und nicht generisch kompensierbar."),
...
class ProjectOSUserManagementReversibilityPolicy:
    def __init__(self, rules: tuple[ProjectOSUserManagementReversibilityRule, ...] = _DEFAULT_RULES) -> None:
        ...
        if len(by_operation) != len(rules): raise ValueError("reversibility operation already configured")
    ...
    def rule(self, operation: str) -> ProjectOSUserManagementReversibilityRule:
        ...
        if key not in self._rules: raise ValueError(f"reversibility policy not configured: {key}")
        return self._rules[key]

    def is_reversible(self, operation: str) -> bool: return self.rule(operation).reversible
    ...
    def require(self, operation: str, *, compensation: str | None = None) -> ProjectOSUserManagementReversibilityRule:
        ...
        if not rule.reversible: raise ValueError(f"operation is not reversible: {operation}")
        if compensation is not None and rule.compensation != compensation: raise ValueError("reversibility compensation does not match policy")
        return rule

    def state(self) -> dict[str, Any]:
        return {"rules": [rule.as_dict() for rule in self._rules.values()],
                "reversible_operations": [rule.operation for rule in self._rules.values() if rule.reversible],
                "read_only": True, "persisted": False, "fail_closed": True}
