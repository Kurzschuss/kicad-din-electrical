...
def test_sync_import_is_undoable_and_updates_dirty_state(tmp_path: Path):
    manager = _manager()
    sync_service = DinEditorSyncService(manager.change_service)
    manager.save(tmp_path / "integration.json")
...
def test_resolve_all_is_one_undoable_operation(tmp_path: Path):
    manager = _manager()
    actions = _actions(manager)
    actions.inspect([
...
def test_invalid_sync_log_entry_has_clear_error():
    from .din_editor_project_bundle import import_project_bundle
    invalid_entries = [{}, {"timestamp": "2026-07-31T18:00:00+00:00"}, {"timestamp": "2026-07-31T18:00:00+00:00", "reference": "X5", "source": "KiCad", "value": "24V", "action": 42}, "not-an-entry"]
...
def test_invalid_sync_log_timestamp_has_clear_error():
    ...
    for timestamp in ["not-a-timestamp", "2026-07-31T18:00:00", "2026-07-31 18:00:00"]:
        try:
            import_project_bundle({"version": 2, "session": {"version": 1, "components": []}, "sync_log": [{"timestamp": timestamp, **base}]})
        except DinProjectBundleError as exc:
            assert "invalid DIN editor project data" in str(exc)
        else:
...
def test_sync_log_timestamp_accepts_timezone_aware_iso8601():
    from .din_editor_project_bundle import import_project_bundle
    entry = {"timestamp": "2026-07-31T18:00:00+00:00", "reference": "X5", "source": "KiCad", "value": "24V", "action": "imported"}
    _, log = import_project_bundle({"version": 2, "session": {"version": 1, "components": []}, "sync_log": [entry]})
