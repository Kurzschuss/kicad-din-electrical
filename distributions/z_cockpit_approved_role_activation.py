...
_STATUS_LABELS = {"risk_not_configured": "Risikoklasse nicht konfiguriert", "approval_missing": "Freigabeauftrag fehlt",
                  "pending_approval": "Freigabe ausstehend", "approved": "Freigegeben",
                  "approved_not_required": "Keine zweite Freigabe erforderlich", "rejected": "Abgelehnt",
                  "emergency_pending_review": "Notfall vorläufig wirksam – Nachprüfung erforderlich"}


class ZCockpitApprovedRoleActivationView:
    def __init__(self, *, project_id: str, user: ProjectOSUserProfile,
        ...
        self.project_id = project_id; self.user = user; self.base_assignments = tuple(base_assignments or ())
        ...
        self.evaluator = ProjectOSApprovedRoleActivationEvaluator(
            roles=roles, activations=activations, role_terminations=role_terminations,
            approval_requests=approval_requests, approvals=approvals, risk_class_map=risk_class_map,
            user_deactivations=self.user_deactivations)

    @staticmethod
    def _decorate_approval(item: dict[str, Any]) -> dict[str, Any]:
        result = dict(item); result["approval"] = dict(result["approval"]); status = result["approval"].get("status")
        result["approval"]["status_label"] = _STATUS_LABELS.get(status, str(status)); return result
    ...
    def state(self, *, scope: str = "project", at: datetime | None = None) -> dict[str, Any]:
        ...
        derived = self.evaluator.permission_assignments(project_id=self.project_id, user=self.user,
            permission_map=self.permission_map, scope=scope, at=at)
        auth = ZCockpitAuthorizationView(self.user, self.base_assignments + derived, user_deactivations=self.user_deactivations)
        ...
        termination_approvals = {
            ...
            "configuration_required": bool(termination_state.get("configuration_required")), "read_only": True}
        ...
        return {
            "project_id": approval_state["project_id"], "user": self.user.as_dict(), "scope": scope,
            "effective_activations": [self._decorate_approval(item) for item in approval_state["effective_activations"]],
            "blocked_activations": blocked_activations, "pending_post_reviews": pending_reviews,
            "terminated_assigned_roles": list(approval_state["terminated_assigned_roles"]),
            "role_assignment_termination_approvals": termination_approvals, "rights": rights,
            ...
            "attention_required": bool(blocked_activations or pending_reviews or termination_approvals["blocked_terminations"]
                                       or termination_approvals["pending_post_reviews"] or termination_approvals["configuration_required"]),
            "read_only": True}
