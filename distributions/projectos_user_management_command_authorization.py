...
class ProjectOSUserManagementCommandAuthorization:
    ...
    def _role_assignments(self, user: ProjectOSUserProfile, *, at: datetime | None) -> tuple[ProjectOSPermissionAssignment, ...]:
        ...
        activation_evaluator = ProjectOSApprovedRoleActivationEvaluator(
            ...
            approvals=state.approvals,
            risk_class_map=self.role_risk_class_map,
            user_deactivations=state.user_deactivations,
            user_reactivations=state.user_reactivations,
        )
        candidates = activation_evaluator.permission_assignments(
            project_id=state.project_id,
            user=user,
            permission_map=self.role_permission_map,
            scope=self.scope,
            at=at,
        )
        effective = []
    ...
    def _granting_role_termination_diagnostics(
        ...
        *,
        ...
        role_ids = {
            ...
            and item.scope == self.scope
            and item.role_type in granting_role_types
        }
    ...
    def evaluate(
        ...
        *,
        ...
        state = self.manager.user_management
        lifecycle = self._user_lifecycle(actor.user_id, at)
        ...
        if lifecycle["deactivated"]:
            return {
                ...
                "role_derived_assignment_count": 0,
                "terminated_granting_role_count": 0,
                "blocked_granting_role_termination_count": 0,
                "role_termination_configuration_required": False,
                "user_lifecycle_status": lifecycle["status"],
                "user_deactivated": True,
                "user_deactivation": authorization["user_deactivation"],
                "user_reactivation": None,
                "deny_precedence": True,
                "weight_used_for_decision": False,
                "read_only": True,
            }
        ...
        return {
            ...
            "user_deactivation": None,
            "user_reactivation": authorization.get("user_reactivation"),
