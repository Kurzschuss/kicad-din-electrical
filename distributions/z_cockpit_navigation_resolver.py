...
class ZCockpitNavigationResolver:
    ...
    def __init__(self, manager: DinEditorProjectManager, messages: Iterable[ProjectOSMessageEnvelope] | None = None, memory: ProjectOSProjectMemory | None = None) -> None:
        self.manager = manager
        self.messages = tuple(messages or ())
        self.memory = memory
        self._correlation_view = ZCockpitProjectCorrelationView(manager, messages=self.messages, memory=memory)

    def resolve_context(self, context: ZCockpitNavigationContext) -> dict[str, Any]:
        resolved = self.resolve(context.current)
        return {**resolved, "navigation": context.as_dict()}
    ...
    def resolve(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        ...
        handlers = {
            ...
            "user_management_consistency": self._user_management_consistency,
        }
        payload = handlers[target.view](target)
        return {"target": target.as_dict(), "resolved_view": target.view, "payload": payload, "read_only": True}

    def _project_overview(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        return ZCockpitProjectLeadOverview(self.manager, messages=self.messages, memory=self.memory).state(correlation_id=target.correlation_id)
    ...
    def _audit(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        ...
        if target.audit_filter == "unresolved_causation":
            message_ids = {m.message_id for m in self.messages if m.project_id == self.manager.project_id}
            entries = [e for e in entries if e.get("causation_id") and e.get("causation_id") not in message_ids]
        elif target.audit_filter == "unlinked":
            entries = [e for e in entries if not e.get("correlation_id")]
        elif target.audit_filter not in {None, "all"}:
            raise ValueError(f"unsupported audit_filter: {target.audit_filter}")
        return {**audit, "entries": entries, "entry_count": len(entries), "applied_filter": target.audit_filter}
    ...
    def _knowledge_diagnostics(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        ...
        message_ids = {m.message_id for m in self.messages if m.project_id == self.manager.project_id}
        correlation_ids = {m.correlation_id for m in self.messages if m.project_id == self.manager.project_id}
        state = ZCockpitDiagnosticsWorklistView(memory, known_message_ids=message_ids, known_correlation_ids=correlation_ids).state(correlation_id=target.correlation_id, role="project_lead")
        state["focus"] = {"knowledge_ids": list(target.knowledge_ids), "relation_ids": list(target.relation_ids)}
        return state

    def _knowledge_element(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        memory = self._require_memory()
        knowledge_id = target.knowledge_ids[0]
        visible = {e.knowledge_id: e for e in memory.elements(correlation_id=target.correlation_id)}
        if knowledge_id not in visible:
            raise ValueError("knowledge element is not visible in this scope")
        return {"element": visible[knowledge_id].as_dict(), "relations": [r.as_dict() for r in memory.relations(knowledge_id=knowledge_id)], "correlation_id": target.correlation_id, "read_only": True}

    def _knowledge_path(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        self._require_memory()
        return self._correlation_view.explain_knowledge_path(target.knowledge_ids[0], target.knowledge_ids[1], correlation_id=target.correlation_id)

    def _knowledge_origin(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        self._require_memory()
        return ZCockpitKnowledgeOriginEvidenceView(self._correlation_view).state(target.knowledge_ids[0], correlation_id=target.correlation_id)
    ...
    def _approval_trace(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        ...
        return ZCockpitRoleApprovalTraceView(messages=self.messages, audit_entries=self.manager.sync_log.export()).state(project_id=self.manager.project_id, correlation_id=target.correlation_id, action_id=target.metadata.get("action_id"))
    ...
    def _user_management_consistency(self, target: ZCockpitNavigationTarget) -> dict[str, Any]:
        return ZCockpitUserManagementConsistencyView(self.manager).state()
